...
let transactions = [];
let cashOuts = []; // Variabel penampung kas keluar
...
function loadCatalogFromCloud() {
    if(productGrid) productGrid.innerHTML = '<p style="text-align:center; grid-column:1/-1; color:gray; font-size:13px; margin-top:20px;">Memuat data dari sistem...</p>';
    ...
    fetch(urlAntiNyangkut)
        ...
        .then(data => { 
            ...
            renderFinance(); // Update saldo saat data termuat
        })
        .catch(() => {
            if(productGrid) productGrid.innerHTML = '<p style="text-align:center; grid-column:1/-1; color:red; font-size:13px;">Gagal memuat sistem.</p>';
        });
}
...
checkoutBtn.addEventListener('click', () => {
    ...
    .then(() => {
        if (custWA && custWA.length >= 9) {
            window.resendWA(invoiceNumber, custWA, custName, itemDetails, total, paymentStatus);
        } else {
        ...
        renderCart(); 
        loadCatalogFromCloud(); // Tarik data terbaru agar saldo otomatis diupdate
        switchView('dashboardView');
    })
...
// --- RIWAYAT TRANSAKSI & BLUETOOTH PRINTER ---
...
window.printThermalReceipt = async function(invoice, name, itemsStr, total, cash, change, status) {
    try {
        ...
        if (!device) { alert("Tidak ada printer yang dipilih."); return; }
        ...
        for (const service of services) {
            ...
            for (const characteristic of characteristics) {
                if (characteristic.properties.write || characteristic.properties.writeWithoutResponse) { targetCharacteristic = characteristic; break; }
            }
            if (targetCharacteristic) break;
        }
        if (!targetCharacteristic) { alert("Gagal menemukan jalur data printer."); return; }
        ...
        itemsStr.split(', ').forEach(item => { commands.push(encoder.encode(`- ${item}\n`)); });
        ...
        for (let cmd of commands) { await targetCharacteristic.writeValue(cmd); }
        alert("Nota berhasil dicetak!"); server.disconnect();
    } catch (error) {
        alert("Pencetakan dibatalkan / gagal koneksi.");
    }
};
...
if(saveCoBtn) {
    saveCoBtn.addEventListener('click', () => {
        ...
        .then(() => { 
            ...
            loadCatalogFromCloud(); // Tarik data terbaru agar saldo otomatis update
        })
